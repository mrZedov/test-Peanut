...
@Injectable()
export class ExchangeBinanceService implements ExchangeApi {
  public exchangeName = EExchange.Binance;
  ...
  public async getTicker(data: GetRatesRequestDto): Promise<GetRatesResponseDto> {
    ...
    const response = await axios.get(`${this.baseUrl}ticker/price?symbol=${paramExchangeRequest.symbol}`);
